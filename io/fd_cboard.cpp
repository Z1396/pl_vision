...
{
/**
 * @brief FD_CBoard类的构造函数，初始化控制板相关资源和状态
 * @param config_path YAML配置文件路径，用于加载CAN总线参数等配置
 */
FD_CBoard::FD_CBoard(const std::string & config_path)
// 初始化列表：按顺序初始化类成员变量
: mode(Mode::idle),                  // 初始化工作模式为空闲状态（Mode是自定义枚举）
  ...
  can_(read_yaml(config_path), std::bind(&FD_CBoard::callback, this, std::placeholders::_1))
 ...
 */
Eigen::Quaterniond FD_CBoard::imu_at(std::chrono::steady_clock::time_point timestamp)
{
...
void FD_CBoard::send(FD_Send_Command command) const
{
    ...
    fd_frame.flags = 0;  // 0表示CAN FD帧（若设为CANFD_BRS可启用比特率切换）
    fd_frame.len = 14;  // 数据长度为14字节（根据FD_Command结构体的实际数据量设置）
    fd_frame.data[0] = (command.control) ? 1 : 0;
 ...
 */
void FD_CBoard::callback(const canfd_frame & frame)
{
...
// 实现方式有待改进
std::string FD_CBoard::read_yaml(const std::string & config_path)
{
