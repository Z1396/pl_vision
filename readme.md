...
│   └── ...
├── src            // 应用层，见3.4软件架构
│   └── auto_aim_debug_mpc.cpp	    MPC 自瞄调试
        auto_buff_debug_mpc.cpp	    MPC 打符调试
        auto_buff_debug.cpp	        基础打符调试
        mt_auto_aim_debug.cpp	    多线程自瞄调试
        mt_standard.cpp	            多线程标准程序
        sentry.cpp	                哨兵主程序
        sentry_bp.cpp	            哨兵备用程序
        standard.cpp	            标准主程序
        sentry_multithread.cpp	    多线程哨兵程序
        standard_mpc.cpp	        MPC 标准程序
...
步骤一:开机如果没写can配置脚本需要自己手动配置can波特率与驱动
    #usb_to_can可能需要的指令(识别、can收发器)
         ...
         # 1. 卸载可能冲突的驱动（如之前装过的 PEAK 官方驱动）
            sudo modprobe -r pcan

            # 2. 加载 SocketCAN 核心模块
            sudo modprobe can_dev  # CAN 设备核心驱动
            sudo modprobe can_raw  # 原始 CAN 帧支持
            

            # 3. 加载 PEAK 设备专用驱动，强制生成 can0 接口
            sudo modprobe peak_usb dev=can0

            # 4. 验证接口是否生成
            ip link show can0
            # 正常输出：3: can0: <NOARP,ECHO> mtu 16 qdisc noop state DOWN ...


        # 1. 配置 CAN 波特率（必须和总线上其他设备一致！）
