...
: classifier_(config_path), debug_(debug)
{
  ...
  threshold_ = yaml["threshold"].as<double>();                     //基础检测阈值（如二值化阈值，用于分割图像中的高亮区域，如装甲板灯条）。
  max_angle_error_ = yaml["max_angle_error"].as<double>() / 57.3;  //灯条角度误差上限
  min_lightbar_ratio_ = yaml["min_lightbar_ratio"].as<double>();   //灯条长宽比下限
  max_lightbar_ratio_ = yaml["max_lightbar_ratio"].as<double>();   //灯条长宽比上限
  min_lightbar_length_ = yaml["min_lightbar_length"].as<double>(); //灯条最小长度。过滤过短的噪点（如小光斑）
  min_armor_ratio_ = yaml["min_armor_ratio"].as<double>();         //装甲板长宽比下限。
  max_armor_ratio_ = yaml["max_armor_ratio"].as<double>();         //装甲板长宽比上限。
  max_side_ratio_ = yaml["max_side_ratio"].as<double>();           //装甲板两侧灯条长度比例上限。正常装甲板两侧灯条长度应接近，差异过大则过滤。
  min_confidence_ = yaml["min_confidence"].as<double>();           //检测置信度下限
  max_rectangular_error_ = yaml["max_rectangular_error"].as<double>() / 57.3;  //装甲板矩形度误差上限（弧度）。装甲板近似矩形，边缘角度偏差过大则过滤。
...
Color Detector::get_color(const cv::Mat & bgr_img, const std::vector<cv::Point> & contour) const
{
  ...
  for (const auto & point : contour) {
    blue_sum += bgr_img.at<cv::Vec3b>(point)[2];
    red_sum += bgr_img.at<cv::Vec3b>(point)[0];
  }
